...
import os
import argparse
OUTPUT_FILE_PATH = os.path.dirname(os.path.abspath(__file__)) + "/output.txt"
...
def output(info_sub_file, rule: dict, status: bool = True, output_file_path=OUTPUT_FILE_PATH):
    ...
    with open(output_file_path, "a") as f:
        if status:
            #write to file
            f.write(info_sub_file)
            f.write("\n")
            f.write("rule_id: " + rule['rule_id'] + "\n")
            f.write("message: " + rule['message'] + "\n")
            f.write("cwe: " + str(rule['cwe']) + "\n\n")
            ...
            #write to file
            f.write("No malicious commits were found\n\n")
    f.close()
...
###################### test by run all file code ######################
    ...
    for i in range(len(lst_sub_file_content)):
        ...
        f.write("\n\n")
        f.close()
        processing(lst_sub_file_content[i], rules_folder=rules_folder, output_file_path=output_file_path)
